...
parser.add_argument('--n_trm_layers', default=2, type=int, help='number of transformer layers')
parser.add_argument("--hidden_dropout_prob", type=float, default=0.3, help="hidden dropout p")
parser.add_argument('--n_attention_heads', default=4, type=int, help='number of heads in attention')
...
def main():
    ...
    model = trans_to_cuda(BCH(adjacency=train_data.adjacency,n_node=n_node,lr=opt.lr, l2=opt.l2, beta=opt.beta,
                               lam=opt.lam, eps=opt.eps, layers=opt.layer,emb_size=opt.embSize,
                               batch_size=opt.batchSize, dataset=opt.dataset, n_trm_layers = opt.n_trm_layers,
                               hidden_dropout_prob = opt.hidden_dropout_prob, n_attention_heads = opt.n_attention_heads,
